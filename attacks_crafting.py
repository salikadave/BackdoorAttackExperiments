...
from src.utils import pattern_craft, add_backdoor, pattern_craft_patch
...
perturbation = pattern_craft_patch(trainset.__getitem__(0)[0].size(), config['MASK_SIZE'], config['MARGIN'], config['PATCH_TYPE'])
...
for i in ind_train:
    if train_images_attacks is not None:
        train_images_attacks = torch.cat([train_images_attacks, add_backdoor(trainset.__getitem__(i)[0], perturbation, type='patch').unsqueeze(0)], dim=0) # (TO DO)
        train_labels_attacks = torch.cat([train_labels_attacks, torch.tensor([config["TC"]], dtype=torch.long)], dim=0)
    else:
        train_images_attacks = add_backdoor(trainset.__getitem__(i)[0], perturbation, type='patch').unsqueeze(0)
...
for i in ind_test:
    if test_images_attacks is not None:
        test_images_attacks = torch.cat([test_images_attacks, add_backdoor(testset.__getitem__(i)[0], perturbation, type='patch').unsqueeze(0)], dim=0)
        test_labels_attacks = torch.cat([test_labels_attacks, torch.tensor([config["TC"]], dtype=torch.long)], dim=0)
    else:
        test_images_attacks = add_backdoor(testset.__getitem__(i)[0], perturbation, type='patch').unsqueeze(0)
...
torch.save(train_attacks, f'./attacks/patch/train/train_attacks_{config['MASK_SIZE']}_{config['MARGIN']}')
torch.save(test_attacks, f'./attacks/patch/test/test_attacks_{config['MASK_SIZE']}_{config['MARGIN']}')
torch.save(ind_train, f'./attacks/patch/ind/ind_train_{config['MASK_SIZE']}_{config['MARGIN']}')
